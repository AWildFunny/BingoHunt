# 随机分队（3人决战）——手动调用入口
# 用法：/function bingofinal:random_team_3

tellraw @a {"text":"[BingoFinal] random_team_3 begin","color":"yellow"}
...
execute if score #need bf.tmp matches 0 run tellraw @a {"text":"[BingoFinal] 未检测到参赛玩家（bf.index 未设置）。请先执行 /function bingofinal:3_dual 放置基地。","color":"red"}
...
execute as @a[scores={bf.index=0..2}] store result score @s bf.rand run random value 0..1
execute as @a[scores={bf.index=0..2}] if score @s bf.rand matches 1 run scoreboard players set @s bf.role 2

# 确保至少一猎物与一猎人
scoreboard players set #prey bf.tmp 0
execute as @a[scores={bf.index=0..2,bf.role=2}] run scoreboard players add #prey bf.tmp 1
execute if score #prey bf.tmp matches 0 as @a[scores={bf.index=0..2},limit=1,sort=random] run scoreboard players set @s bf.role 2
scoreboard players set #hun bf.tmp 0
execute as @a[scores={bf.index=0..2,bf.role=1}] run scoreboard players add #hun bf.tmp 1
execute if score #hun bf.tmp matches 0 as @a[scores={bf.index=0..2},limit=1,sort=random] run scoreboard players set @s bf.role 1
...
# 打印分队结果
execute as @a[scores={bf.index=0..2,bf.role=1}] run tellraw @a {"text":"[猎人] ","color":"red","extra":[{"selector":"@s"}]}
execute as @a[scores={bf.index=0..2,bf.role=2}] run tellraw @a {"text":"[猎物] ","color":"green","extra":[{"selector":"@s"}]}

# 结束提示
tellraw @a {"text":"[BingoFinal] 随机分配完成，游戏开始！","color":"gold"}